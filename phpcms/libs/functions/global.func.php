...
function remove_xss($string) { 
    ...
    $parm1 = Array('javascript', 'vbscript', 'expression', 'applet', 'meta', 'xml', 'blink', 'link', 'script', 'embed', 'object', 'siframes', 'sframe', 'frameset', 'ilayer', 'layer', 'bgsound', 'title', 'base');
 ...
 */
 function trim_script($str) {
	if(is_array($str)){
		...
		$str = preg_replace ( '/\<([\/]?)siframe([^\>]*?)\>/si', '&lt;\\1iframe\\2&gt;', $str );
 ...
 *@param array()
 **/
function safe_array_string($dataarray=array()){
		foreach($dataarray as $k=>$v){
			$dataarray[$k]=htmlspecialchars(addslashes(trim_script(trim($dataarray[$k]))));
		}
		return $dataarray;
}
 ...
 *@param array()
 **/
function safe_one_string($datastring=''){
	$datastring=htmlspecialchars(addslashes(trim_script(safe_replace(trim($datastring)))));
	return $datastring;
}
...
//十进制转十六进制
function ten_to_chex($number,$n=0){
	if($n>0){
		$result = str_pad(dechex($number),$n,'0',STR_PAD_LEFT);
	}else{
...
//排序
function more_array_sort($arr,$keys,$type='asc'){ 
