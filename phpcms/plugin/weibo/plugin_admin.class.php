		...
		public function oauth() {
			...
			if(!isset($_REQUEST['oauth_verifier']) || $_REQUEST['oauth_verifier'] == '') {
				if($this->auth_db->get_one(array('source'=>'sina'))) {
					$txt = '授权成功，<a href="?m=admin&c=plugin&a=config&pluginid='.$this->pluginid.'&module=removeauth&checkhash='.$_SESSION['checkhash'].'">解除绑定</a>';
				} else {
				...
				$txt = '授权成功，'.$me['name'].'<a href="?m=admin&c=plugin&a=config&pluginid='.$this->pluginid.'&module=removeauth&checkhash='.$_SESSION['checkhash'].'">解除绑定</a>';
			}
