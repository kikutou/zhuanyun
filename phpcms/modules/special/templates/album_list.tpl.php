...
<input type="hidden" value="<?php echo $_GET['checkhash'];?>" name="checkhash">
...
?>   
	...
	<td align="center"><span style="height:22"><?php if (!empty($imported) && in_array($info['id'], $imported)){?><font color="red">专辑已载入<?php } else {?><a href='?m=special&c=album&a=import&id=<?php echo $info['id']?>&page=<?php echo $_GET['page'];?>&checkhash=<?php echo $_GET['checkhash'];?>'>载入此专辑</a><?php }?></span><br /><span style="height:22"><a href="?m=special&c=album&a=content_list&id=<?php echo $info['id']?>&menuid=<?php echo $_GET['menuid']?>&checkhash=<?php echo $_GET['checkhash'];?>">查看专辑下的视频</a></span></td>
	</tr>
<?php 
	}
}
