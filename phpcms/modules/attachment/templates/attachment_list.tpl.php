...
<td width="20%"><img src="<?php echo file_icon($info['filename'],'gif')?>" /> <?php echo $info['filename']?> <?php echo $thumb ? '<img title="'.L('att_thumb_manage').'" src="resource/images/admin_img/havthumb.png" onclick="showthumb('.$info['aid'].', \''.new_addslashes($info['filename']).'\')"/>':''?> <?php echo $info['status'] ? '<img src="resource/images/admin_img/link.png"':''?></td>
...
function att_delete(obj,aid){
	...
	function(){
	$.get('?m=attachment&c=manage&a=delete&aid='+aid+'&checkhash=<?php echo $_SESSION['checkhash']?>',function(data){
