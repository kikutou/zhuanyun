...
class coupon_tag {
	...
	public function pc_tag() {
		...
		if(is_array($result)) {
			$specials = array(L('please_select', '', 'coupon'));
			foreach($result as $r) {
		...
		return array(
			'action'=>array('lists'=>L('lists', '', 'coupon')),
			'lists'=>array(
				'siteid'=>array('name'=>L('sitename', '', 'coupon'),'htmltype'=>'input_select', 'defaultvalue'=>get_siteid(), 'data'=>$sitelist),
			),
		);
	}
}
