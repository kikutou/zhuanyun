...
class admin_coupon extends admin {
	...
	public function __construct() {
		...
		$this->db = pc_base::load_model('coupon_model');
	}
	...
	public function init() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=coupon&c=admin_coupon&a=add\', title:\''.L('coupon_add').'\', width:\'600\', height:\'350\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('coupon_add'));
	...
	public function add() {
		...
		if(isset($_POST['dosubmit'])) {
			$_POST['coupon'] = $this->check($_POST['coupon']);
			if($this->db->insert($_POST['coupon'])) showmessage(L('couponment_successful_added'), HTTP_REFERER, '', 'add');
		} else {
			//获取站点模板信息
	...
	public function edit() {
		...
		if(isset($_POST['dosubmit'])) {
			$_POST['coupon'] = $this->check($_POST['coupon'], 'edit');
			if($this->db->update($_POST['coupon'], array('aid' => $_GET['aid']))) showmessage(L('coupond_a'), HTTP_REFERER, '', 'edit');
		} else {
	...
	public function public_approval($aid = 0) {
		if((!isset($_POST['aid']) || empty($_POST['aid'])) && !$aid) {
			...
			if(is_array($_POST['aid']) && !$aid) {
				...
				showmessage(L('coupon_passed'), HTTP_REFERER);
			} elseif($aid) {
				$aid = intval($aid);
	...
	public function delete($aid = 0) {
		if((!isset($_POST['aid']) || empty($_POST['aid'])) && !$aid) {
			...
			if(is_array($_POST['aid']) && !$aid) {
				...
				showmessage(L('coupon_deleted'), HTTP_REFERER);
			} elseif($aid) {
				$aid = intval($aid);
	...
	private function check($data = array(), $a = 'add') {
		...
		if($data['code']=='') showmessage(L('couponments_cannot_be_empty'));
		if($data['symbol']=='') showmessage(L('couponsymbol_cannot_be_empty'));
		if($data['exchangerate']=='') showmessage(L('couponexchangerate_cannot_be_empty'));
		$r = $this->db->get_one(array('title' => $data['title']));
		
		if ($a=='add') {
			if (is_array($r) && !empty($r)) {
				showmessage(L('coupon_exist'), HTTP_REFERER);
			}
