...
function creat_form($id, $data, $value = '', $op = '') {
	...
	if($data['ajax']['name']) {
		if($data['ajax']['m']) {
			$url = '$.get(\'?m=content&c=push&a=public_ajax_get\', {html: this.value, id:\''.$data['ajax']['id'].'\', action: \''.$data['ajax']['action'].'\', module: \''.$data['ajax']['m'].'\', checkhash: \''.$_SESSION['checkhash'].'\'}, function(data) {$(\'#'.$id.'_td\').html(data)});';
		} else {
			$url = '$.get(\'?m=template&c=file&a=public_ajax_get\', { html: this.value, id:\''.$data['ajax']['id'].'\', action: \''.$data['ajax']['action'].'\', op: \''.$op.'\', style: \'default\', checkhash: \''.$_SESSION['checkhash'].'\'}, function(data) {$(\'#'.$id.'_td\').html(data)});';
		}
	}
...
function visualization($html, $style = '', $dir = '', $file = '') {
	$change = "<link href=\"".CSS_PATH."dialog.css\" rel=\"stylesheet\" type=\"text/css\" />
		...
		var checkhash = '".$_SESSION['checkhash']."';
		$(function(){
