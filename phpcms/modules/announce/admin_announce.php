...
class admin_announce extends admin {
	...
	public function init() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\'}).close();window.top.art.dialog({id:\'add\',iframe:\'?m=announce&c=admin_announce&a=add\', title:\''.L('announce_add').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('announce_add'));
	...
	public function public_check_title() {
		...
		$title = $_GET['title'];
		if ($_GET['aid']) {
			$r = $this->db->get_one("aid!='".$_GET['aid']."' and title='$title'");
			if ($r) {
				exit('0');
			}else{
				exit('1');;
