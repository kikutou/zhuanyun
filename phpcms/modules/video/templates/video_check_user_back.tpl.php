...
<form name="myform" action="?m=video&c=video&a=check_user_back&checkhash=<?php echo $_GET['checkhash'];?>" method="post" id="myform">
<fieldset>
...
<!--填写skey_sn-->
<br>
<form name="myform2" action="?m=video&c=video&a=setting&checkhash=<?php echo $_GET['checkhash'];?>" method="post" id="myform2">
<fieldset>
...
function send_code() {
	var email = $("#email").val(); 
	var checkhash = "<?php echo $_GET['checkhash'];?>";
	if(email==''){
		alert('email 不能为空！');return false;
	}
	$.get('?m=video&c=video&a=send_code&checkhash='+checkhash,{ email:email,random:Math.random()}, function(data){
		if(data==1) { 
