...
<input type="submit" value="<?php echo L('search')?>" class="button" name="dosubmit">
 ...
 <form name="myform" id="myform" action="index.php?m=video&c=import&a=doimport&checkhash=<?php echo $_GET['checkhash'];?>" method="post" >
<div class="table-list">
    <table width="100%">
        <thead>
            <tr>
...
function select_pos(obj) {
	...
	var hash = '<?php echo $_GET['checkhash']?>';
	$('#posid').html('<img src="<?php echo IMG_PATH.'msg_img/loading.gif';?>">');
	$.get("index.php", {m:'video', c:'video', a:'public_get_pos', catid:catid, tm:Math.random(), checkhash:hash}, function (data) {
		if (data) {
			$('#posid').html(data);
		} else {
