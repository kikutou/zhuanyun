...
<!--
	...
	parent.document.getElementById('center_frame').src = '?m=content&c=content&a=public_categorys&type=add&menuid=<?php echo $_GET['menuid'];?>&checkhash=<?php echo $_SESSION['checkhash'];?>';
...
<a class="add fb" href="javascript:;" onclick=javascript:openwinx('?m=content&c=content&a=add&menuid=&catid=<?php echo $catid;?>&checkhash=<?php echo $_SESSION['checkhash'];?>','')><em><?php echo L('add_content');?></em></a>　
<a href="?m=content&c=content&a=init&catid=<?php echo $catid;?>&checkhash=<?php echo $checkhash;?>" <?php if($steps==0 && !isset($_GET['reject'])) echo 'class=on';?>><em><?php echo L('check_passed');?></em></a><span>|</span>
...
<span>|</span><a href="?m=content&c=create_html&a=category&pagesize=30&dosubmit=1&modelid=0&catids[0]=<?php echo $catid;?>&checkhash=<?php echo $checkhash;?>&referer=<?php echo urlencode($_SERVER['QUERY_STRING']);?>"><em><?php echo L('update_htmls',array('catname'=>$category['catname']));?></em></a>
<?php }?>
</div>
...
<input type="hidden" value="<?php echo $checkhash;?>" name="checkhash">
...
<tbody>
	...
	?>
		...
		<?php
		if($r['sysadd']==0) {
			echo "<a href='?m=member&c=member&a=memberinfo&username=".urlencode($r['username'])."&checkhash=".$_SESSION['checkhash']."' >".$r['username']."</a>"; 
...
function update_param(fid, obj) {
	...
	$.get('index.php', {m:'content', c:'content', a:'update_param', field:field, id:id, value:value, modelid:modelid, checkhash:'<?php echo $_SESSION['checkhash'];?>'}, function (data){
		if (data!=200){
            $(obj).val(old_data);
		}
	})
