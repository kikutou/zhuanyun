...
class site extends admin {
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$billprefix = isset($_POST['billprefix']) && trim($_POST['billprefix']) ? trim($_POST['billprefix']) : '';

			$site_title = isset($_POST['site_title']) && trim($_POST['site_title']) ? trim($_POST['site_title']) : '';

			$qqno = isset($_POST['qqno']) && trim($_POST['qqno']) ? trim($_POST['qqno']) : '';
			$public_tel = isset($_POST['public_tel']) && trim($_POST['public_tel']) ? trim($_POST['public_tel']) : '';
			...
			$package_overday = isset($_POST['package_overday']) ? trim($_POST['package_overday']) : 0;

			$introducer = isset($_POST['introducer']) ? intval($_POST['introducer']) : 0;
			$regperson = isset($_POST['regperson']) ? intval($_POST['regperson']) : 0;
			$autofinsh = isset($_POST['autofinsh']) ? intval($_POST['autofinsh']) : 0;
			
			$wbinfocustom = isset($_POST['wbinfocustom']) ? intval($_POST['wbinfocustom']) : 0;

			$package_overdayfee = isset($_POST['package_overdayfee']) ? floatval($_POST['package_overdayfee']) : 0;
			...
			if ($this->db->insert(array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'release_point'=>$release_point, 'template'=>$template,'setting'=>$setting, 'default_style'=>$default_style,'billprefix'=>$billprefix,'package_overday'=>$package_overday,'qqno'=>$qqno,'introducer'=>$introducer,'regperson'=>$regperson,'autofinsh'=>$autofinsh,'package_overdayfee'=>$package_overdayfee,'public_tel'=>$public_tel,'wbinfocustom'=>$wbinfocustom,'copyright'=>$copyright,'logo'=>$logo))) {
