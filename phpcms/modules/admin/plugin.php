	...
	public function import() {
		if(!isset($_GET['dir'])) {
			...
			if(empty($_GET['license']) && $license) {
				$submit_url = '?m=admin&c=plugin&a=import&dir='.$dir.'&license=1&checkhash='. $_SESSION['checkhash'].'&menuid='.$_GET['menuid'];
			} else {
				$submit_url = '?m=admin&c=plugin&a=install&dir='.$dir.'&checkhash='. $_SESSION['checkhash'].'&menuid='.$_GET['menuid'];
			}	
				$show_header = 0;
