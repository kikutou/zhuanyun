...
class cache_api {
	...
	//update storage cache method
	public function get__storage__lists(){
		
		//列出所会员仓库
		$storagedb = pc_base::load_model('storage_model');
		$warehouse__lists = $storagedb->select("siteid=1 ",'*',10000,'listorder asc');
		
		setcache('get__storage__lists', $warehouse__lists, 'commons');
		return true;
	}

	//update enum cache method 
	public function get__enum__lists(){
		$enumdb = pc_base::load_model('enum_model');
		$enum_lists = $enumdb->select(array('groupid'=>0,'siteid'=>1),'*',1000,'listorder asc');
		
		foreach($enum_lists as $v){
			$aid = $v['aid'];
			
			$groupname="enum_data_".$aid;
			
			delcache('get__'.$groupname, $data_lists, 'commons');
			
			$addsql="";	
			if($aid==9)//order status
			{
				$addsql=" AND (remark!='0' or remark is null)";
			}
			
			$data_lists = $enumdb->select("groupid='".$aid."' ".$addsql,'*',1000,'listorder asc');

			setcache('get__'.$groupname, $data_lists, 'commons');
			
		}
		
		$ldb = pc_base::load_model('linkage_model');
		$_area_data_lists = $ldb->select("parentid=0 AND keyid=0 ",'*',10000,'listorder asc');

		foreach($_area_data_lists as $v){

			
				$groupname="enum_data_48_".trim($v['linkageid']);
				$data_lists2 = $enumdb->select("groupid='48' and country='".trim($v['name'])."' ".$addsql,'*',1000,'listorder asc');
				setcache('get__'.$groupname, $data_lists2, 'commons');

				$groupname123="enum_data_123_".trim($v['linkageid']);
				$data_lists2_123 = $enumdb->select("groupid='123' and country='".trim($v['name'])."' ".$addsql,'*',1000,'listorder asc');
				setcache('get__'.$groupname123, $data_lists2_123, 'commons');
		}
	...
	//update turnway cache method
	public function get__turnway__lists(){
		
		//列出所会员仓库
		$turnwaydb = pc_base::load_model('shipline_model');
		$data_lists = $turnwaydb->select("siteid=1 ",'*',10000,'aid asc');
		
		setcache('get__turnway__lists', $data_lists, 'commons');
		return true;
	}

	//update linkage cache method
	public function get__linkage__lists(){
		
		//列出所会员仓库
		$ldb = pc_base::load_model('linkage_model');
		$data_lists = $ldb->select("parentid=0 AND keyid=0 ",'*',10000,'listorder asc');
		
		setcache('get__linkage__lists', $data_lists, 'commons');
		return true;
	}

	//update service cache method
	public function get__service__lists(){
		
		//列出所会员仓库
		$ldb = pc_base::load_model('service_model');
		$data_lists = $ldb->select("siteid=1 AND type='order'",'*',10000,'aid asc');
		
		setcache('get__service__lists', $data_lists, 'commons');
		return true;
	}

	public function get__addvalueservice__lists(){
		
		//列出所会员仓库
		$ldb = pc_base::load_model('service_model');
		$data_lists = $ldb->select("siteid=1 AND type='value-added'",'*',10000,'aid asc');
		
		setcache('get__addvalueservice__lists', $data_lists, 'commons');
		return true;
	}
	
	//update currency cache method
	public function get__currency__lists(){
		
		//列出所会员仓库
		$ldb = pc_base::load_model('currency_model');
		$data_lists = $ldb->select("siteid=1",'*',10000,'aid asc');
		
		setcache('get__currency__lists', $data_lists, 'commons');
		return true;
	}
