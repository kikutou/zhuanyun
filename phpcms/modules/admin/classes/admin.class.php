...
class admin {
	...
	final public static function submenu($parentid = '', $big_menu = false) {
		...
		foreach($array as $_value) {
			...
			if($classname) {
				...
				$string .= "<a href='?m=".$_value['m']."&c=".$_value['c']."&a=".$_value['a']."&menuid=$parentid&checkhash=$checkhash".'&'.$_value['data']."' $classname><em>".L($_value['name'])."</em></a><span>|</span>";
			}
		}
	...
	final private function check_hash() {
		...
		if(isset($_GET['checkhash']) && $_SESSION['checkhash'] != '' && ($_SESSION['checkhash'] == $_GET['checkhash'])) {
			return true;
		} elseif(isset($_POST['checkhash']) && $_SESSION['checkhash'] != '' && ($_SESSION['checkhash'] == $_POST['checkhash'])) {
			return true;
		} else {
