	...
	public function init() {
		if (isset($_POST['dosubmit']) || isset($_GET['dosubmit'])) {
			...
			$modules = array(
				...
				array('name' => L('storage'), 'function' => 'get__storage__lists'),
				array('name' => L('enum'), 'function' => 'get__enum__lists'),
				array('name' => L('turnway'), 'function' => 'get__turnway__lists'),
				array('name' => 'All'.L('linkage'), 'function' => 'get__linkage__lists'),
				array('name' => L('service'), 'function' => 'get__service__lists'),
				array('name' => 'addvalue'.L('service'), 'function' => 'get__addvalueservice__lists'),
				array('name' => L('currency'), 'function' => 'get__currency__lists'),
			);
			...
			if (!empty($modules[$page])) {
				...
				showmessage(L('update').$m['name'].L('cache_file_success'), '?m=admin&c=cache_all&page='.$page.'&dosubmit=1&checkhash='.$_SESSION['checkhash'], 0);
			} else {
