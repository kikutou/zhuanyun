...
class module extends admin {
	...
	public function install() {
		...
		if ($_POST['dosubmit']) {
			if ($module_api->install()) showmessage(L('success_module_install').L('update_cache'), '?m=admin&c=module&a=cache&checkhash='.$_SESSION['checkhash']);
	...
	public function uninstall() {
		...
		else showmessage(L('uninstall_success'), '?m=admin&c=module&a=cache&checkhash='.$_SESSION['checkhash']);
	}
	...
	public function cache() {
		echo '<script type="text/javascript">parent.right.location.href = \'?m=admin&c=cache_all&a=init&checkhash='.$_SESSION['checkhash'].'\';window.top.art.dialog({id:\'install\'}).close();</script>';
