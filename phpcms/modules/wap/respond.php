...
class respond {
	...
	public function respond_get() {
		if ($_GET['code']){
			...
			if(!$payment) showmessage("支付方式发生错误");
			...
			if($return_data) {
				...
				showmessage("恭喜您，支付成功",APP_PATH.'index.php?m=wap&c=deposit');
			} else {
				showmessage("支付失败，请联系管理员",APP_PATH.'index.php?m=wap&c=deposit');
			}
		} else {
			showmessage("恭喜您，支付成功");
		}
	}
	...
	public function respond_post() {
		...
		if ($_POST['code']){
			...
			if(!$payment) error_log(date('m-d H:i:s',SYS_TIME).'| '.$_POST['code'].' POST: payment is null |'."\r\n", 3, CACHE_PATH.'pay_error_log.php');;
			$cfg = unserialize_config($payment['config']);
			$pay_name = ucwords($payment['pay_code']);
			pc_base::load_app_class('pay_factory','pay',0);
			$payment_handler = new pay_factory($pay_name, $cfg);
	...
	private function update_waybill_status_by_sn($trade_sn){
		...
		if($bill && $orderinfo){
			...
			if(!empty($waybillid)){
				$msg = $_mrow['username'].'已成功付款运单：'.$waybillid;
