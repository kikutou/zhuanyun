...
$ord = $_GET['ord'];
	...
	include template('wap', 'orderno');
	}
		
	public function about(){
		$catid=2;
	
		$pdb = pc_base::load_model('page_model');

		$row = $pdb->get_one(array('catid'=>$catid));
	...
	public function news() {
		...
		$catid = isset($_GET['catid']) ? intval($_GET['catid']) : 13;
				

		include template('wap', "news");
	}	
	...
	function big_image() {
		...
		$WAP_SETTING = string2array($WAP['setting']);
		$GLOBALS['siteid'] = max($this->siteid,1);		
		$new_url=base64_decode(trim($_GET['url']));
