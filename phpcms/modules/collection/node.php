...
class node extends admin {
	...
	public function col_content() {
		...
		if ($data = $this->db->get_one(array('nodeid'=>$nodeid))) {
			...
			if ($total_page > $page) {
				showmessage(L('collectioning').($i+($page-1)*2).'/'.$total.'<script type="text/javascript">location.href="?m=collection&c=node&a=col_content&page='.($page+1).'&nodeid='.$nodeid.'&total='.$total.'&checkhash='.$_SESSION['checkhash'].'"</script>', '?m=collection&c=node&a=col_content&page='.($page+1).'&nodeid='.$nodeid.'&total='.$total);
			} else {
				$this->db->update(array('lastdate'=>SYS_TIME), array('nodeid'=>$nodeid));
	...
	public function import_content() {
		...
		$url = '?m=collection&c=node&a=publist&nodeid='.$nodeid.'&status=2&checkhash='.$_SESSION['checkhash'];
		if ($type == 'all') {
		...
		if ($type == 'all' && $total_page > $page) {
			$str = L('are_imported_the_import_process').(($page-1)*20+$i).'/'.$total.'<script type="text/javascript">location.href="?m=collection&c=node&a=import_content&nodeid='.$nodeid.'&programid='.$programid.'&type=all&page='.($page+1).'&total='.$total.'&checkhash='.$_SESSION['checkhash'].'"</script>';
			$url = '';
		}
		showmessage($str, $url);
	}
