...
<fieldset>
	...
	<?php foreach($forminfos as $k=>$v) {?>
		<tr>
			...
			<td><?php echo $v['form'];if($k=='idcard_fb' || $k=='idcard_fe'){echo '<br><a id="'.$k.'_ahref" target=_blank><img src="" id="'.$k.'_img" border=0 style="display:none;" width="500" /></a><script>if($("#'.$k.'").val()!=""){$("#'.$k.'_img").attr("src",$("#'.$k.'").val());$("#'.$k.'_ahref").attr("href",$("#'.$k.'").val());$("#'.$k.'_img").show();}</script>';};?></td>
		</tr>
	<?php }?>
	</table>
</fieldset>
...
<!--
	function changemodel(modelid) {
		redirect('?m=member&c=member&a=edit&userid=<?php echo $memberinfo[userid]?>&modelid='+modelid+'&checkhash=<?php echo $_SESSION['checkhash']?>');
	}
//-->
</script>
</html>