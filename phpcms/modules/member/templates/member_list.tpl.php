...
<input type="hidden" value="" name="act" id="actact">
    ...
    <tbody>
		...
		<div class="explain-col">
				...
				<select name="type">
					...
					<option value='6' <?php if(isset($_GET['type']) && $_GET['type']==6){?>selected<?php }?>>真实姓名</option>
					<option value='7' <?php if(isset($_GET['type']) && $_GET['type']==7){?>selected<?php }?>>用户标识</option>
				</select>
				
				<input name="keyword" type="text" value="<?php if(isset($_GET['keyword'])) {echo $_GET['keyword'];}?>" class="input-text" />
				<input type="submit" name="search" class="button" value="<?php echo L('search')?>" />  <span style="float:right"><input type="submit" class="button" name="downloadsearch" onclick="document.getElementById('actact').value='download';" value="导出"/></span>
	</div>
		</td>
		</tr>
    </tbody>
...
<?php
	...
	foreach($memberlist as $k=>$v) {
		$udetail = $m__db->get_one(array('userid'=>$v['userid']));
?>
    <tr>
		...
		<td align="left"><?php echo $v['clientname']?></td>
		<td align="left"><?php echo $v['clientcode']?></td>
		...
		<td align="left"><?php echo htmlspecialchars($udetail['lastname'].$udetail['firstname'])?></td>
