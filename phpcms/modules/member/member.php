...
class member extends admin {
	...
	function search() {
		...
		if (isset($_GET['search']) || isset($_GET['downloadsearch'])) {
			...
			if($keyword) {
				if ($type == '1') {
					...
					if(strpos($keyword," ")!==false){
						...
						$sqlkey=" and ((firstname='".trim($arrkey[1])."' and lastname='".trim($arrkey[0])."'  ) or (lastname='".trim($arrkey[1])."' and firstname='".trim($arrkey[0])."'  ))";
					}
					$where .= "`userid` in(select userid from t_member_detail where 1 $sqlkey)";
				} else {
					$where .= "`username` like '%$keyword%'";
				}
			} else {
		...
		if($act=='download'){
			...
			//标头 
			//用户ID	用户名	昵称	真实姓名	邮箱	会员组	注册ip	最后登录	金钱总数	积分点数
			$excelHeader=array('用户ID','用户名','昵称','真实姓名','邮箱','会员组','注册ip','最后登录','金钱总数','积分点数','电话','添加时间','国家','省/州','城市','详细地址','邮编','固定电话','即时通讯','身份证号码');
			...
			$addsql="";
			if($where){$addsql=" where ";}
			//数据
			$wsql="select * from t_member $addsql ".$where." order by userid desc ";
			$result = $this->db->query($wsql);
			$allrecords = $this->db->fetch_array($result);
			$rownum=1;
			foreach($allrecords  as $num=>$row) {
				$rownum+=1;
			
				$usrow = $m_db->get_one(array('userid'=>$row['userid']));
				$objActSheet->setCellValueByColumnAndRow(0,$rownum,$row['userid']);
				$objActSheet->setCellValueByColumnAndRow(1,$rownum,$row['username']);
				$objActSheet->setCellValueByColumnAndRow(2,$rownum,$row['nickname']);
				$objActSheet->setCellValueByColumnAndRow(3,$rownum,$usrow['lastname'].$usrow['firstname'].'('.$usrow['truename'].')');
				$objActSheet->setCellValueByColumnAndRow(4,$rownum,$row['email']);
				$objActSheet->setCellValueByColumnAndRow(5,$rownum,$grouplist[$row['groupid']]);
				$objActSheet->setCellValueByColumnAndRow(6,$rownum,$row['regip']);
				$objActSheet->setCellValueByColumnAndRow(7,$rownum,format::date($row['lastdate'], 1));
				$objActSheet->setCellValueByColumnAndRow(8,$rownum,$row['amount']);
				$objActSheet->setCellValueByColumnAndRow(9,$rownum,$row['point']);
				$objActSheet->setCellValueByColumnAndRow(10,$rownum,$row['mobile']);
				$objActSheet->setCellValueByColumnAndRow(11,$rownum,date('Y-m-d H:i:s',$row['regdate']));

				$objActSheet->setCellValueByColumnAndRow(12,$rownum,$usrow['country']);
				$objActSheet->setCellValueByColumnAndRow(13,$rownum,$usrow['province']);
				$objActSheet->setCellValueByColumnAndRow(14,$rownum,$usrow['city']);
				$objActSheet->setCellValueByColumnAndRow(15,$rownum,$usrow['address']);
				$objActSheet->setCellValueByColumnAndRow(16,$rownum,$usrow['postcode']);
				$objActSheet->setCellValueByColumnAndRow(17,$rownum,$usrow['telephone']);
				$objActSheet->setCellValueByColumnAndRow(18,$rownum,$usrow['nowconnect']);
				$objActSheet->setCellValueByColumnAndRow(19,$rownum,'#'.$usrow['idcard']);
			}
			...
			header("Content-Disposition: attachment;filename=\"".iconv("UTF-8","GB2312","会员列表").date('ymdHis').".xls\"");
	...
	function add() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			/////////////////////////////////////////////////////////////////////////////////////////////////////
			$info['clientname'] = get__rand__char(5);
			/* 插入订单表 */
			$error_no = $m__db->count(array('clientname'=>$info['clientname']));
			do
			{
					$info['clientname'] = get__rand__char(5); //获取新订单号
					$error_no = $m__db->count(array('clientname'=>$info['clientname']));
			}
			while ($error_no > 0 ); //如果是订单号重复则重新提交数据

			$info['clientcode'] = get__rand__char(6,1);
			/* 插入订单表 */
			$error_no = $m__db->count(array('clientcode'=>$info['clientcode']));
			do
			{
					$info['clientcode'] = get__rand__char(6,1); //获取新订单号
					$error_no = $m__db->count(array('clientcode'=>$info['clientcode']));
			}
			while ($error_no > 0 ); //如果是订单号重复则重新提交数据
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function memberinfo() {
		...
		foreach($model_fieldinfo as $k=>$v) {
			if($v['formtype'] == 'image') {
				$member_modelinfo[$k] = "<a href='.$member_modelinfo[$k].' target='_blank'><img src='.$member_modelinfo[$k].' height='40' widht='40' onerror=\"this.src='$phpsso_api_url/resource/images/member/nophoto.gif'\"></a>";
			} elseif($v['formtype'] == 'images') {
				$tmp = string2array($member_modelinfo[$k]);
				$member_modelinfo[$k] = '';
				if(is_array($tmp)) {
					foreach ($tmp as $tv) {
						$member_modelinfo[$k] .= " <a href='$tv[url]' target='_blank'><img src='$tv[url]' height='40' widht='40' onerror=\"this.src='$phpsso_api_url/resource/images/member/nophoto.gif'\"></a>";
					}
					unset($tmp);
				}
