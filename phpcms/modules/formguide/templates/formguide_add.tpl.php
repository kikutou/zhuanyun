...
<tbody>
	...
	<tr>
		...
		<td id="show_template"><script type="text/javascript">$.getJSON('?m=admin&c=category&a=public_tpl_file_list&style=<?php echo $info['default_style']?>&module=formguide&templates=show&name=info&checkhash='+checkhash, function(data){$('#show_template').html(data.show_template);});</script></td>
	</tr>
	<tr>
		<th><strong>js调用使用的模板：</strong></th>
		<td id="show_js_template"><script type="text/javascript">$.getJSON('?m=admin&c=category&a=public_tpl_file_list&style=<?php echo $info['default_style']?>&module=formguide&templates=show_js&name=info&checkhash='+checkhash, function(data){$('#show_js_template').html(data.show_js_template);});</script></td>
	</tr>
	</tbody>
</table>
