...
<table width="100%" cellspacing="0" class="search-form">
 ...
 <table width="100%" cellspacing="0" class="search-form">
	...
	?>
	&nbsp;&nbsp;
<a href="<?php echo APP_PATH;?>index.php?m=dianping&c=dianping&a=dianping_data&menuid=<?php echo $_GET['menuid'];?>&checkhash=G6Ihra&typeid=<?php echo $type;?>"><?php echo $type_array['type_name'];?></a>	
	<?php }}?>
		</div>
		</td>
		</tr>
...
function check(id, type, dianpingid) {
	...
	$.get('?m=dianping&c=dianping&a=ajax_checks&id='+id+'&type='+type+'&dianpingid='+dianpingid+'&'+Math.random()+'&checkhash=<?php echo $_SESSION['checkhash'];?>', function(data){if(data!=1){if(data==0){alert('<?php echo L('illegal_parameters')?>')}else{alert(data)}}else{$('#tbody_'+id).remove();}});
}
</script>
</body>
</html>