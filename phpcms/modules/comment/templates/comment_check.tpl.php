...
function check(id, type, commentid) {
	...
	$.get('?m=comment&c=check&a=ajax_checks&id='+id+'&type='+type+'&commentid='+commentid+'&checkhash='+checkhash+'&'+Math.random(), function(data){if(data!=1){if(data==0){alert('<?php echo L('illegal_parameters')?>')}else{alert(data)}}else{$('#tbody_'+id).remove();

	$.getJSON('?m=comment&c=check&a=public_get_one'+'&checkhash='+checkhash+Math.random(), function(data){
		if (data) {
