...
function chkselpackage(){
	...
	if(str==""){
		...
		window.top.art.dialog({content:"<?php echo L('please_select').L('splitbox_package').L('package');?>",lock:true,width:'220',height:'70'}, function(){});
...
function getusername(userid){
	if(userid){
		$.post("?m=package&c=admin_package&a=splitbox&checkhash=<?php echo $_SESSION['checkhash'];?>&uid="+userid,function(data){
			$("#username").html(data);
		});
		$.post("?m=package&c=admin_package&a=splitbox&checkhash=<?php echo $_SESSION['checkhash'];?>&tid="+userid,function(data){
			$("#truename").html(data);
		});
	}
}
