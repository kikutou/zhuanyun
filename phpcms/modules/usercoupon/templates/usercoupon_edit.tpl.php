...
?>
<div class="pad-10">
<form method="post" action="?m=usercoupon&c=admin_usercoupon&a=edit&aid=<?php echo $_GET['aid']?>" name="myform" id="myform">
<table class="table_form" width="100%">
	...
	<tr>
		...
		<td><input name="usercoupon[number]" id="number" class="input-text" type="text" size="10" value="<?php echo ($an_info['number'])?>">张/人 </td>
	</tr>
	<tr>
		<th><strong><?php echo L('okdays')?>：</strong></th>
		<td>从今日起至<input name="usercoupon[okdays]" id="okdays" class="input-text" type="text" size="8" value="<?php echo ($an_info['okdays'])?>"> 天后过期。</td>
	</tr>
	<tr>
  		<th><strong><?php echo L('remark')?>：</strong></th>
        <td>
		<textarea name="usercoupon[remark]" id="remark" class="input-text" cols="60" rows="5"><?php echo ($an_info['remark'])?></textarea>
		</td>
	</tr>
	</tbody>
...
function load_file_list(id) {
	$.getJSON('?m=admin&c=category&a=public_tpl_file_list&style='+id+'&module=usercoupon&templates=show&name=usercoupon&checkhash='+checkhash, function(data){$('#show_template').html(data.show_template);});
}

$(document).ready(function(){
	$.formValidator.initConfig({formid:"myform",autotip:true,onerror:function(msg,obj){window.top.art.dialog({content:msg,lock:true,width:'220',height:'70'}, function(){this.close();$(obj).focus();})}});
	$('#title').formValidator({onshow:"<?php echo L('input_usercoupon_title')?>",onfocus:"<?php echo L('title_min_3_chars')?>",oncorrect:"<?php echo L('right')?>"}).inputValidator({min:1,onerror:"<?php echo L('title_cannot_empty')?>"}).ajaxValidator({type:"get",url:"",data:"m=usercoupon&c=admin_usercoupon&a=public_check_title&aid=<?php echo $_GET['aid']?>",datatype:"html",cached:false,async:'true',success : function(data) {
        if( data == "1" )
		{
            return true;
		}
