...
<div class="pad-10">
    <form
        action="?m=maillist&c=maillist&a=maillist_create&menuid=<?php echo $menuid;?>&checkhash=<?php echo $checkhash;?>"
        ...
        <input type="hidden" name="checkhash" id="checkhash" value="<?php echo $checkhash;?>" />
		...
		if ($('#is_activate').val() == '1') {
			$.ajax({
	    		...
	    		success : function(data){
					...
					location.href = '?m=maillist&c=maillist&a=maillist_mgr&menuid=' + $('#menuid').val() + '&checkhash=' + $('#checkhash').val();
	    		}
			});
			return false;
		}
