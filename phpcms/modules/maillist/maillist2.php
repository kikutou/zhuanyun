...
class maillist extends admin {
	...
	function send_setting() {
		if ($_SERVER ['REQUEST_METHOD'] != 'POST') {
			...
			$pos_url = $app_url . 'index.php?m=maillist&c=maillist&a=send_setting&menuid=' . $menuid . '&checkhash=' . $checkhash;
	...
	function user_mgr() {
		if ($_SERVER['REQUEST_METHOD'] != 'POST') {
			...
			$hash = md5(join('', $params) . 'o.sdo');
			$app_url = pc_base::load_config('system', 'app_path');
			$pos_url = $app_url . 'index.php?m=maillist&c=maillist&a=user_mgr&menuid=' . $menuid . '&checkhash=' . $checkhash;
			$en_url = base64_encode($pos_url);
