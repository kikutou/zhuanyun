	...
	public function pay_recharge() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($payment['is_online'] === '1') {
				...
				$payment_handler->set_productinfo($product_info)->set_orderinfo($order_info)->set_customerinfo($customer_info);
				if($oid>0)
					$code = $payment_handler->get_code('value="确认付款" class="btn-login radius-three"');
				else
					$code = $payment_handler->get_code('value="确认充值" class="btn-login radius-three"');	
			} else {
