...
class payment extends admin {
	private $db, $account_db, $member_db,$account_historydb;
	function __construct() {
		if (!module_exists(ROUTE_M)) showmessage(L('module_not_exists')); 
		parent::__construct();
		$this->db = pc_base::load_model('pay_payment_model');
		$this->account_db = pc_base::load_model('pay_account_model');
		$this->account_historydb = pc_base::load_model('account_history_model');
	...
	public function payment_history() {
		...
		if($_GET['dosubmit']){
			...
			if($trade_sn) $where = "AND orderid LIKE '%$trade_sn%' ";
		...
		$infos = $this->account_historydb->listinfo($where, $order = 'addtime DESC', $page, $pagesize = 20);
		$pages = $this->account_historydb->pages;
		$number = count($infos);
		include $this->admin_tpl('payment_history');	
	}
	...
	public function modify_deposit() {
		if(isset($_POST['dosubmit'])) {
			$money_type = trim($_POST['money_type']);
			...
			if($userinfo) {	
				...
				if($_POST['pay_unit']) {
					...
					$receipts->$func($value, $userinfo['userid'] , $username, create_sn(), 'offline', $payment, param::get_cookie('admin_username'), $status = 'succ',$usernote,$money_type);					
					...
					$func = $_POST['pay_type'] == '1' ? 'amount' :'point';
					$spend->$func($value,$msg,$userinfo['userid'],$username,param::get_cookie('userid'),param::get_cookie('admin_username'),'',($money_type)   );
				...
				if(intval($_POST['sendemail'])) {
					...
					$msg = L('account_changes_notice_tips',array('username'=>$username,'time'=>date('Y-m-d H:i:s',SYS_TIME),'op'=>$op,'note'=>$usernote,'amount'=>(floatval($userinfo['amount'])+$value),'point'=>$userinfo['point']));
