...
class receipts {
	 ...
	 */
	public static function amount($value, $userid = '' , $username = '', $trade_sn = '', $pay_type = '', $payment = '', $op_username = '', $status = 'succ', $note = '',$money_type=0) {
		return self::_add(array('username'=>$username, 'userid'=>$userid,'money'=>$value, 'trade_sn'=>$trade_sn, 'pay_type'=>$pay_type, 'payment'=>$payment, 'status'=>$status, 'type'=>1, 'adminnote'=>$op_username, 'usernote'=>$note,'money_type'=>$money_type));
	}
	 ...
	 * @param string  $op_username 管理员信息
	 */
	public static function point($value, $userid = '' , $username = '', $trade_sn = '', $pay_type = '', $payment = '', $op_username = '', $status = 'succ', $note = '',$money_type=0) {
		return self::_add(array('username'=>$username, 'userid'=>$userid,'money'=>$value, 'trade_sn'=>$trade_sn, 'pay_type'=>$pay_type, 'payment'=>$payment, 'status'=>$status, 'type'=>2, 'adminnote'=>$op_username, 'usernote'=>$note,'money_type'=>$money_type));
	}
	...
	private static function _add($data) {
		...
		$data['ip'] = ip();
		$data['money_type'] = isset($data['money_type']) && intval($data['money_type']) ? intval($data['money_type']) : 0;
