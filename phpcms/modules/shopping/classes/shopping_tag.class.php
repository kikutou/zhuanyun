...
class shopping_tag {
 	private $shopping_db,$type_db;
	
	public function __construct() {
		$this->shopping_db = pc_base::load_model('shopping_model');
	...
	public function lists($data) {
		$typeid = intval($data['typeid']);//分类ID
 		$shoppingtype = $data['shoppingtype']? $data['shoppingtype'] : 0;
		$siteid = $data['siteid'];
		if (empty($siteid)){ 
			$siteid = get_siteid();
		}
  		if($typeid!='' || $typeid=='0'){
 				$sql = array('typeid'=>$typeid,'shoppingtype'=>$shoppingtype,'siteid'=>$siteid,'passed'=>'1');
			}else {
				$sql = array('shoppingtype'=>$shoppingtype,'siteid'=>$siteid,'passed'=>'1');
		}
  		$r = $this->shopping_db->select($sql, '*', $data['limit'], 'listorder '.$data['order']);
		return new_html_special_chars($r);
	}
	...
	public function type_list($data) {
		...
		$r = $this->shopping_db->select($sql, '*', $data['limit'], $data['order']);
		return new_html_special_chars($r);
	}
	...
	public function get_typelist($siteid='1', $value = '', $id = '') {
   			$data = $arr = array();
			$data = $this->type_db->select(array('module'=>'shopping', 'siteid'=>$siteid));
	...
	public function count($data) {
		if($data['action'] == 'lists') {
			...
			if($typeid!='' || $typeid=='0'){
 			...
 			return $this->shopping_db->count($sql); 
		}
	}
	...
	public function pc_tag() {
		...
		return array(
			'action'=>array('type_list'=>L('shopping_list', '', 'shopping')),
			'type_list'=>array(
				...
				'shoppingtype'=>array('name'=>L('shopping_type','','shopping'), 'htmltype'=>'select', 'data'=>array('0'=>L('word_shopping','','shopping'), '1'=>L('logo_shopping','','shopping'))),
