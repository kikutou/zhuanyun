...
class house_tag {
	...
	public function pc_tag() {
		...
		if(is_array($result)) {
			$specials = array(L('please_select', '', 'house'));
			foreach($result as $r) {
		...
		return array(
			'action'=>array('lists'=>L('lists', '', 'house')),
			'lists'=>array(
				'siteid'=>array('name'=>L('sitename', '', 'house'),'htmltype'=>'input_select', 'defaultvalue'=>get_siteid(), 'data'=>$sitelist),
			),
		);
	}
}
