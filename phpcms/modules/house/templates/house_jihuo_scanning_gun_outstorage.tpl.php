...
function buildfile(){
	...
	$.get("?m=house&c=admin_house&a=house_jihuo&hid=<?php echo $this->hid?>&flag=7&t="+Math.random()+"&checkhash=<?php echo $_SESSION['checkhash'];?>&status=-1&start_time="+beginT+"&end_time="+endT,function(data){
...
function setFos(){
	if(window.top.art.dialog({id:'scanning_gun_outstorage'})){
		window.top.art.dialog({id:'scanning_gun_outstorage'}).data.iframe.document.getElementById('BatchNo').focus();
	}
}
