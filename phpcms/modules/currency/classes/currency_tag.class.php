...
class currency_tag {
	...
	public function pc_tag() {
		...
		if(is_array($result)) {
			$specials = array(L('please_select', '', 'currency'));
			foreach($result as $r) {
		...
		return array(
			'action'=>array('lists'=>L('lists', '', 'currency')),
			'lists'=>array(
				'siteid'=>array('name'=>L('sitename', '', 'currency'),'htmltype'=>'input_select', 'defaultvalue'=>get_siteid(), 'data'=>$sitelist),
			),
		);
	}
}
