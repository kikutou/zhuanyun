...
class enum_tag {
	...
	public function pc_tag() {
		...
		if(is_array($result)) {
			$specials = array(L('please_select', '', 'enum'));
			foreach($result as $r) {
		...
		return array(
			'action'=>array('lists'=>L('lists', '', 'enum')),
			'lists'=>array(
				'siteid'=>array('name'=>L('sitename', '', 'enum'),'htmltype'=>'input_select', 'defaultvalue'=>get_siteid(), 'data'=>$sitelist),
			),
		);
	}
}
