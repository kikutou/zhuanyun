...
class turnway_tag {
	...
	public function pc_tag() {
		...
		if(is_array($result)) {
			$specials = array(L('please_select', '', 'turnway'));
			foreach($result as $r) {
		...
		return array(
			'action'=>array('lists'=>L('lists', '', 'turnway')),
			'lists'=>array(
				'siteid'=>array('name'=>L('sitename', '', 'turnway'),'htmltype'=>'input_select', 'defaultvalue'=>get_siteid(), 'data'=>$sitelist),
			),
		);
	}
}
