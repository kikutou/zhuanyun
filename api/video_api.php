...
if(!empty($_GET['do_complete']) && !empty($_GET['uid']) && !empty($_GET['snid'])){
	...
	showmessage("请完善注册信息资料！",APP_PATH.'index.php?m=video&c=video&a=complete_info&uid='.intval($_GET['uid']).'&snid='.$_GET['snid'].'&checkhash='.$checkhash);
}

if(!empty($_GET['skey']) && !empty($_GET['sn'])){
	header("Location: ".APP_PATH.'index.php?m=video&c=video&a=set_video_setting&skey='.$_GET['skey'].'&sn='.$_GET['sn'].'&checkhash='.$checkhash); 
}
