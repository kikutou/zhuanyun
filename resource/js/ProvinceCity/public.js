...
(function() {
	$(".proCitySelAll").click(function(event) {
		...
		var t2 = o2.top;
		var h2 = $(this).height();
		$(".provinceCityAll").css("top", "26").css("left", "0").toggle();
...
//		}
		for (i = 0; i < allCitys.cities.length; i++) {
			if (v.toUpperCase() === allCitys.cities[i].cityPinyin.substring(0, v.length)) {
				htmlArr[htmlArr.length] = "<li><a class='allcityClass' href='javascript:' provinceId=" + allCitys.cities[i].provinceId + " cityId=" + allCitys.cities[i].id + ">" + allCitys.cities[i].provinceName+" - "+allCitys.cities[i].name + " (<span style='color:red'>" + v.toUpperCase() + "</span>" + allCitys.cities[i].cityShortPY.substring(v.length, allCitys.cities[i].cityShortPY.length) + ")</a></li>";
				if (htmlArr.length > 9) {
					break;
					return false;
				}
			...
			if (v === allCitys.cities[i].name.substring(0, v.length)) {
				htmlArr[htmlArr.length] = "<li><a class='allcityClass' href='javascript:' provinceId=" + allCitys.cities[i].provinceId + " cityId=" + allCitys.cities[i].id + ">"+allCitys.cities[i].provinceName+" - <span style='color:red'>" + v + "</span>" + allCitys.cities[i].name.substring(v.length, allCitys.cities[i].name.length) + " (" + allCitys.cities[i].cityShortPY + ")</a></li>";
				if (htmlArr.length > 19) {
					break;
					return false;
				}
				autoWidth = autoWidth < (allCitys.cities[i].name + allCitys.cities[i].cityShortPY).length ? (allCitys.cities[i].name + allCitys.cities[i].cityShortPY).length: autoWidth;
			...
			if (v === allCitys.cities[i].provinceName.substring(0, v.length)) {
				htmlArr[htmlArr.length] = "<li><a class='allcityClass' href='javascript:'provinceId=" + allCitys.cities[i].provinceId + " cityId=" + allCitys.cities[i].id + "><span style='color:red'>" + v + "</span>" + allCitys.cities[i].provinceName.substring(v.length, allCitys.cities[i].provinceName.length)+" - "+allCitys.cities[i].name + " (" + allCitys.cities[i].cityShortPY + ")</a></li>";
				if (htmlArr.length > 19) {
					break;
					return false;
				}
...
function sendCitiesAjax() {
	$.ajax({
		type: "get",
		url: '../js/ProvinceCity/queryAllCities.js',
		async: true,
		dataType: "json",
...
function sendAllProvinceAjax() {
	$.ajax({
		type: "get",
		url: '../js/ProvinceCity/queryAllProvinces.js',
		async: true,
		dataType: "json",
		success: function(data) {
...
function sendAllCitiesAjax() {
	$.ajax({
		type: "get",
		url: '../js/ProvinceCity/queryCities.js',
		async: true,
		dataType: "json",
		success: function(data) {
...
function sendAllCountiesAjax()
 {
	$.ajax({
		type: "get",
		url: "../js/ProvinceCity/queryAllAreas.js",
		async: true,
		dataType: "json",
		success: function(data) {
			allAreas = data.areas;
...
function viewAllProvince(page) {
	...
	for (var i = start; i < end; i++) {
		...
		if (allProvinces[i].provinceName == '内蒙古自') {
			p_name = '内蒙古';
		} else if (allProvinces[i].provinceName == '黑龙江') {
			p_name = '黑龙江';
		} else {
...
function viewAllCounties(i) {
	...
	$(".provinceCityAll").find(".con").find(".countyAll").hide(); 
	allCountyPage(1);
	...
	if (nameValue == 'template.caddrProCity')
	 {
	   ...
	   $(".provinceCityAll").hide();//没有数据时隐藏区域列表
